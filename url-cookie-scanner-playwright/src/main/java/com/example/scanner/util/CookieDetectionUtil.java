package com.example.scanner.util;

import com.microsoft.playwright.Page;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.Arrays;
import java.util.List;

public class CookieDetectionUtil {

    private static final Logger log = LoggerFactory.getLogger(CookieDetectionUtil.class);

    public static final List<String> CONSENT_ACCEPT_SELECTORS = Arrays.asList(
            // Basic accept buttons
            "button:has-text('Accept')", "button:has-text('Accept All')", "button:has-text('ACCEPT ALL')",
            "button:has-text('I Accept')", "button:has-text('Agree')", "button:has-text('AGREE')",
            "[data-testid*='accept']", "[data-cy*='accept']", "button[id*='accept']",
            "button[class*='accept']", "[aria-label*='accept']",

            // Cookie specific
            "[id*='cookie-accept']", "[class*='cookie-accept']", ".accept-cookies",

            // Popular CMPs
            "#onetrust-accept-btn-handler", ".fc-primary-button",
            "#CybotCookiebotDialogBodyButtonAccept", ".qc-cmp-button",

            // Generic
            ".btn-accept", ".button-accept", ".consent-accept"
    );

    /**
     * Handle consent banners - CRITICAL for more cookies
     */
    public static boolean handleConsentBanners(Page page, int timeoutMs) {
        boolean consentHandled = false;

        try {
            log.debug("Starting consent banner detection");

            // Try to accept cookies
            for (String selector : CONSENT_ACCEPT_SELECTORS) {
                try {
                    if (page.locator(selector).count() > 0) {
                        log.debug("Found consent element: {}", selector);
                        page.locator(selector).first().click();
                        page.waitForTimeout(2000);
                        log.info("Successfully clicked consent button: {}", selector);
                        consentHandled = true;
                        break;
                    }
                } catch (Exception e) {
                    log.debug("Failed to click selector '{}': {}", selector, e.getMessage());
                }
            }

            // Fallback - try any button with "accept" text
            if (!consentHandled) {
                try {
                    if (page.locator("button").count() > 0) {
                        int buttonCount = page.locator("button").count();
                        for (int i = 0; i < Math.min(buttonCount, 10); i++) {
                            try {
                                String buttonText = page.locator("button").nth(i).textContent();
                                if (buttonText != null &&
                                        (buttonText.toLowerCase().contains("accept") ||
                                                buttonText.toLowerCase().contains("agree") ||
                                                buttonText.toLowerCase().contains("allow"))) {
                                    page.locator("button").nth(i).click();
                                    page.waitForTimeout(1000);
                                    log.info("Clicked consent button by text: {}", buttonText);
                                    consentHandled = true;
                                    break;
                                }
                            } catch (Exception e) {
                                // Continue to next button
                            }
                        }
                    }
                } catch (Exception e) {
                    log.debug("Fallback consent handling failed: {}", e.getMessage());
                }
            }

        } catch (Exception e) {
            log.warn("Error during consent banner handling: {}", e.getMessage());
        }

        log.info("Consent banner handling result: {}", consentHandled);
        return consentHandled;
    }

    /**
     * Simulate user interactions to trigger more cookies
     */
    public static void simulateUserInteractions(Page page, boolean scrollEnabled, boolean clickEnabled, int delayMs) {
        try {
            log.debug("Starting user interaction simulation");

            if (scrollEnabled) {
                // Scroll to trigger analytics and lazy loading
                page.evaluate("window.scrollTo(0, document.body.scrollHeight / 4)");
                page.waitForTimeout(1000);
                page.evaluate("window.scrollTo(0, document.body.scrollHeight / 2)");
                page.waitForTimeout(1000);
                page.evaluate("window.scrollTo(0, document.body.scrollHeight)");
                page.waitForTimeout(2000);
                page.evaluate("window.scrollTo(0, 0)");
                page.waitForTimeout(1000);
                log.debug("Completed scrolling simulation");
            }

            if (clickEnabled) {
                // Try to click some safe elements
                try {
                    // Click search box or input if available
                    if (page.locator("input[type='search'], input[name*='search'], input[placeholder*='search']").count() > 0) {
                        page.locator("input[type='search'], input[name*='search'], input[placeholder*='search']").first().click();
                        page.waitForTimeout(500);
                    }

                    // Click navigation links (but don't navigate)
                    if (page.locator("nav a, .nav a, .navigation a").count() > 0) {
                        // Just hover, don't click to avoid navigation
                        page.locator("nav a, .nav a, .navigation a").first().hover();
                        page.waitForTimeout(500);
                    }
                } catch (Exception e) {
                    log.debug("Error in clicking simulation: {}", e.getMessage());
                }
            }

            // Mouse movements
            page.mouse().move(200, 200);
            page.waitForTimeout(200);
            page.mouse().move(600, 400);
            page.waitForTimeout(200);

        } catch (Exception e) {
            log.warn("Error during user interaction simulation: {}", e.getMessage());
        }
    }

    /**
     * Trigger analytics events to create more cookies
     */
    public static void triggerAnalyticsEvents(Page page) {
        try {
            log.debug("Triggering analytics events");

            page.evaluate("""
                // Google Analytics events
                if (typeof gtag === 'function') {
                    gtag('event', 'page_view', {
                        page_title: document.title,
                        page_location: window.location.href
                    });
                    gtag('event', 'scroll', { percent_scrolled: 50 });
                }
                
                if (typeof ga === 'function') {
                    ga('send', 'pageview');
                    ga('send', 'event', 'scroll', 'percent', 50);
                }
                
                // Facebook Pixel
                if (typeof fbq === 'function') {
                    fbq('track', 'PageView');
                }
                
                // Trigger common events
                window.dispatchEvent(new Event('scroll'));
                window.dispatchEvent(new Event('resize'));
                window.dispatchEvent(new Event('focus'));
                
                console.log('Analytics events triggered');
            """);

            log.debug("Completed analytics event triggering");

        } catch (Exception e) {
            log.debug("Error triggering analytics events: {}", e.getMessage());
        }
    }
}